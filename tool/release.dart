...
/// Automated release script for rdf_mapper_annotations
///
...
void main(List<String> args) async {
  ...
  print('rdf_mapper_annotations release script');
  print('------------------------');
  if (dryRun) {
...
void _printUsage() {
  ...
  print('-------------');
  print(
    'Automates the release process for the rdf_mapper_annotations package.',
  );
  print('');
  print('Usage:');
...
Future<void> _ensureNoDevVersionStrings(String releaseVersion) async {
  ...
  for (final filePath in filesToCheck) {
    ...
    final wrongPubAddPattern = RegExp(
      r'dart pub add rdf_mapper_annotations:[^\s"]+',
    );
    ...
    if (devPattern.hasMatch(content) || wrongPubAddPattern.hasMatch(content)) {
      ...
      content = content.replaceAll(
        wrongPubAddPattern,
        'dart pub add rdf_mapper_annotations',
      );
